...
void searchFiles(HashTable *ht, const char *query) {
    ...
    if (wordIndex > 0) { queryWords[queryWordCount][wordIndex] = '\0'; queryWordCount++; }
    if (queryWordCount == 0) { printf("No valid search terms entered.\n\n"); return; }
        ...
        for (int i = 0; i < fileCount; i++) {
            if (results[i].matchCount == matches) {
                ...
                for (int j = 0; j < results[i].lineCount; j++) {
                    ...
                    while (ln != NULL) {
                        int lineNum = ln->lineNumber, alreadyAdded = 0;
                        for (int k = 0; k < uniqueCount; k++)
                            if (uniqueLines[k] == lineNum) { alreadyAdded = 1; break; }
                    ...
                    if (line) {
                        ...
                        for (int pos = 0; line[pos] != '\0';) {
                            ...
                            for (int k = 0; k < queryWordCount; k++) {
                                int wlen = strlen(queryWords[k]), m = 0;
